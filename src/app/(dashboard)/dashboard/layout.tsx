...
import ModileChatLayout from '@/components/ModileChatLayout'
...
const Layout = async ({ children }: LayoutProps) => {
    ...
    const unseenRequestCount = (await fetchRedis("smembers",
        `user:${session.user.id}:incoming_friend_requests`) as User[]).length

    return <div className='w-full flex h-screen'>
        <div className='md:hidden'>
            <ModileChatLayout friends={friends} session={session}
            sidebarOptions={sidebarOptions} unseenRequestCount={unseenRequestCount}
            />
        </div>
        ...
        <aside className='max-h-screen container py-4 md:py-4 w-full'>
            {children}
        </aside>
    </div>
}
